...
func critical() {
    ...
    diff := float64(endTime - startTime)

    timeString := fmt.Sprintf("[%d] Elapsed (ms): %.6f\n", report.Criticals, diff/conversion)
    // fmt.Println(timeString)
    // fmt.Printf("Diff: %.0f, Conversion: %.6f\n", diff, diff/conversion)
	_, err := benchmarkFile.WriteString(timeString)
    // fmt.Printf("[%d] Wrote %d to benchmark file\n", id, numWritten)
    if err != nil {
...
func main() {
    ...
    benchmarkFile, err := os.Create(fileName)
...
func Host(idArg, hostsArg int, planArg Plan) Report {
    ...
    fileName := fmt.Sprintf("node%d-benchmarking-result.txt", idInput)
    benchmarkFile, _ = os.Create(fileName)
    // fmt.Println("Created benchmark file with name", benchmarkFile.Name())
